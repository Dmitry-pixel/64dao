...
class M4Snapshot(Base):
    ...
    # 043: остальной выход расчёта.
    top_gaps:              Mapped[list] = mapped_column(JSONB, nullable=False, default=list,
                                                        server_default=text("'[]'::jsonb"))
    cause_effect:          Mapped[dict | None] = mapped_column(JSONB(none_as_null=True), nullable=True)
    constraint_detail:     Mapped[dict | None] = mapped_column(JSONB(none_as_null=True), nullable=True)
    metrics:               Mapped[dict | None] = mapped_column(JSONB(none_as_null=True), nullable=True)
    reduced:               Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    created_at:            Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
