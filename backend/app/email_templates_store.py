...
DEFAULT_TEMPLATES: dict[str, dict] = {
    ...
    "repeat_m3": {
        "subject": "Пора повторить диагностику портфеля направлений",
        "body_html": (
            "<p>Здравствуйте{name_part}!</p>"
            "<p>С диагностики портфеля направлений{company_part} прошло около {days_since} дн. "
            "Повторная диагностика покажет, как сместились направления на матрице силы "
            "и изменился ли порядок вложений.{followup_part}</p>"
            "<p>Направления перенесутся из прошлой диагностики — останется проверить цифры "
            "и ответить на вопросы по сегодняшнему положению дел.</p>"
            "<p><a href=\"{app_url}/companies\">Запустить повторную диагностику</a></p>"
        ),
        "description": (
            "Метод 3 «Матрица силы». Отправляется через N дней после последней диагностики "
            "портфеля компании; период — тот же, что на странице «Рассылка». Переменные: "
            "{name}, {name_part}, {company}, {company_part}, {days_since}, {app_url}, "
            "{followup_part} — фраза о том, что повтор входит в стоимость, или пусто."
        ),
    },
    "repeat_m4": {
        "subject": "Пора повторить диагностику «Алмазное колесо»",
        "body_html": (
            "<p>Здравствуйте{name_part}!</p>"
            "<p>С полной диагностики «Алмазное колесо»{company_part} прошло около {days_since} дн. "
            "Повторная диагностика покажет, какие модули подтянулись и сместилось ли "
            "системное ограничение.{followup_part}</p>"
            "<p><a href=\"{app_url}/companies\">Запустить повторную диагностику</a></p>"
        ),
        "description": (
            "Метод 4 «Алмазное колесо». Отправляется через N дней после последней полной "
            "диагностики компании; период — тот же, что на странице «Рассылка». Переменные: "
            "{name}, {name_part}, {company}, {company_part}, {days_since}, {app_url}, "
            "{followup_part} — фраза о том, что повтор входит в стоимость, или пусто."
        ),
    },
