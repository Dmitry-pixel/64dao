...
async def create_assessment(
    ...
    #
    # Если право уже использовано, новая диагностика той же компании — не
    # отказ, а новая первичная: она списывается как обычная платная, получает
    # своё право на повтор и продолжает историю компании в «Динамике».
    # Раньше здесь был отказ 403, и клиент не мог платно переоценить ту же
    # компанию — только завести её под другим названием, разорвав динамику.
    primary = None
    ...
    if body.status in ("completed", "paid") and not method2_payload:
        primary = await db.scalar(
            ...
            # Сначала первичная с неиспользованным повтором, среди равных —
            # самая свежая: после новой платной диагностики повтор относится
            # к ней, а не к первой диагностике компании.
            .order_by(
                (Assessment.followup_used < Assessment.followup_allowed).desc(),
