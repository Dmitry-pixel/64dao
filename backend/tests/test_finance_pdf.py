...
def control_content() -> dict:
    return {
        ...
        "action_package": {
            "line6_yin": {"title": "6. Стратегия", "text": "Стратегическая сессия, целевая модель, KPI."}
        },
        "fin_pattern": {
...
def test_finance_section_present_and_complete():
    ...
    # Маршрут перехода: 1 шаг по подвижной линии 6 (роадмап 2.1)
    assert "Маршрут перехода" in html
    assert "Шаг 1. Линия 6" in html
    assert "Последовательность — рекомендуемая логика" in html
    assert "Состояние после шага" in html
