...
def test_cell_breakdown_printed_for_every_direction(report, html):
    ...
    Веб печатает готовую строку из payload, своей копии не держит, поэтому
    расхождение сторон невозможно по построению. Тест пиньтует точную
    формулировку: её правка должна быть осознанной.
    """
    for item in report["objects"]:
        result = item["result"]
