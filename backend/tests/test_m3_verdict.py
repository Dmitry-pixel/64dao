...
def test_stable_in_weak_zone_means_no_money_will_help():
    ...
    Формулировки при этом разные, и это существенно. У limited рынок средний,
    пересборке есть во что упереться. У exit слабы обе оси: пересобирать
    не во что, и обещать пересборку значит обещать несуществующую опору.
    Раньше обе ячейки несли дословно один текст, и различие стиралось.
    """
    assert verdict_for(_r("low", "mid"))["verdict"] == "Пересборка или выход"
    assert (verdict_for(_r("low", "low"))["verdict"]
            == "Не вкладывать: пересобирать нечем")
