...
1. **[2026-08-21] Три задачи по расписанию на хосте — смотреть их логи, а не «Логи» в админке**
   Do instead: `/var/log/64dao-smoke.log` (проверка сайта, каждые 15 мин), `/var/log/64dao-reconcile.log` (досверка оплат, раз в час), `/var/log/64dao-ca-check.log` (сертификаты, раз в неделю). Раздел «Логи» в админке — это лента событий из БД (регистрации, диагностики, PDF), сообщения задач туда не попадают.

2. **[2026-08-21] live-тесты не годятся как мониторинг прода**
   Do instead: `tests/test_smoke.py` и `TestLiveAPI` помечены маркером `live` и исключены (`addopts = -m "not live"`), а pytest не входит в боевой образ — запустить их на проде нечем. Автопроверку делает `app/jobs/smoke_prod.py`: те же проверки без pytest и без запросов с побочными эффектами (никаких login/register, иначе задача начнёт слать OTP каждые 15 минут).

3. **[2026-08-21] Проверять не только сайт, но и то, что мониторинг умеет кричать**
   Do instead: `docker compose exec -T -e SMOKE_BASE_URL=http://127.0.0.1:9 backend python -m app.jobs.smoke_prod` — все проверки провалятся и уйдёт письмо; следующий обычный запуск пришлёт «сайт снова отвечает» и вернёт состояние в `ok`. Непроверенное оповещение хуже его отсутствия: кажется, что за сайтом следят, а в аварию письма нет.
