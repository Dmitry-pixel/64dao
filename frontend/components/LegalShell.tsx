...
interface LegalShellProps {
  /** Slug текущего документа: 'privacy-policy' | 'user-agreement' | 'personal-data-consent' */
  slug: string
  /** Заголовок страницы */
  title: string
  /** HTML-контент из GET /api/documents/{slug} */
  htmlContent: string
}

const CROSS_LINKS = [
  { slug: 'privacy-policy',        label: 'Политика обработки ПДн' },
  { slug: 'user-agreement',        label: 'Пользовательское соглашение' },
  { slug: 'personal-data-consent', label: 'Согласие на обработку ПДн' },
]

/**
 * LegalShell — визуальная обёртка для трёх правовых страниц.
 * Контент приходит через проп htmlContent из серверного fetch.
 * 'use client' — Next.js App Router требует эту директиву для файлов,
 * использующих next/link и (при необходимости) хуки на стороне клиента.
 */
export default function LegalShell({ slug, title, htmlContent }: LegalShellProps) {
  return (
    <div
    ...
    >
      ...
      <div style={{ maxWidth: 880, margin: '0 auto', padding: '64px 40px 24px' }}>
        ...
        >
          <span>Редакция от 26 июня 2026 г.</span>
          <span>Сайт: 64dao.ru</span>
        </div>
      </div>
      ...
      <footer style={{ background: 'var(--brand-teal)' }}>
        ...
        >
          ...
          {/* Перекрёстные ссылки между правовыми документами */}
          <nav style={{ display: 'flex', flexWrap: 'wrap', gap: 20, fontSize: 13 }}>
            {CROSS_LINKS.map((link) => (
              <Link key={link.slug}
                href={'/documents/' + link.slug}
                style={{
                  color: slug === link.slug ? '#ffffff' : 'rgba(255,255,255,0.85)',
                  textDecoration: 'none',
                  fontWeight: slug === link.slug ? 600 : 400,
                }}
              >
                {link.label}
              </Link>
